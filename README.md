...
curl -X POST http://localhost:3000/transactions/txn-041/retry
```
...
`data/sample.csv` has **50** data rows:

| Category | Count | Purpose |
| --- | --- | --- |
| Valid, unique transactions | 42 | Normal happy-path volume |
| — of which `fail-user` | 2 (`txn-041`, `txn-042`) | Deterministic `FAILED` + retry demo |
| Duplicate `transactionId` | 4 | Idempotency within a single file |
| Invalid rows | 4 | Missing `userId`, missing amount, non-numeric amount, amount `<= 0` |
| **Total** | **50** | |
